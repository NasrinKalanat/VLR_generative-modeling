...
class DiffusionModel(nn.Module):
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        z[t<=0,...]=0       
        pred_img = z*torch.sqrt(posterior_variance)+posterior_mean
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
        return pred_img, x_0
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        tau_i = torch.full((batch,),tau_i,device="cuda",dtype=torch.long)
        tau_isub1 = torch.full((batch,),tau_isub1,device="cuda",dtype=torch.long)
        pred_noise, x_0 = self.model_predictions(img,tau_i)
        if torch.any(tau_isub1<=0): return img, x_0
        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        alpha_tau_i1 = extract(alphas_cumprod, tau_i, img.shape)
        ...
        sigma_tau_i = eta * beta * (1-alpha_tau_isub1) /(1-alpha_tau_i1) 

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        cof_eps_tau_i = torch.sqrt(1 - alpha_tau_isub1 - sigma_tau_i)

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        z=torch.randn(x_0.shape,device="cuda")
        z[tau_i<=0,...]=0
        mu = torch.sqrt(alpha_tau_isub1) * x_0 + cof_eps_tau_i * pred_noise
        img = z * torch.sqrt(sigma_tau_i) + mu
